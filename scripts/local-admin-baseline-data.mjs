...
import { mockWordbookFixture, MOCK_DATASET_ID, MOCK_SCOPE_IDS, MOCK_UNIT_IDS } from "./local-mock-wordbooks-data.mjs";
import { studentQuizFixture } from "./local-quiz-feedback-data.mjs";
import { LOCAL_SCHOOL_KEY, localSchoolSchedulePayload, localSchoolScheduleEdit } from "./local-school-schedule-data.mjs";
...
export function fixtureResponse({ url, method, headers, body = "", quizFeedback = false, studentProfile = false, schoolSchedules = false, mockWordbooks = false }) {
  if (schoolSchedules) students[1].gradeLabel = "고3";
  ...
  const checkedIds = structuredClone(input);
  if (mockWordbooks && method === "POST" && target.pathname === "/rest/v1/rpc/create_mock_wordbook_composition_v1" && checkedIds?.p_request) delete checkedIds.p_request.requestId;
  if (schoolSchedules && method === "POST" && checkedIds) {
    // Request IDs are generated by the browser; school and student identities stay synthetic.
    if (target.pathname === "/rest/v1/rpc/save_admin_school_schedule_event_v1" && checkedIds.p_input?.schoolKey === LOCAL_SCHOOL_KEY) {
      delete checkedIds.p_input.requestId;
      if (/^manual:[0-9a-f-]{36}$/.test(checkedIds.p_input.event?.id ?? "")) checkedIds.p_input.event.id = "manual-local";
    }
  ...
  if (mockWordbooks) { const result = mockWordbookFixture({ target, method, input, headers }); if (result) return result; }
  ...
  if (method === "POST" && table.startsWith("rpc/")) {
    ...
    if (schoolSchedules && ["get_admin_school_schedule_editor_v1", "save_admin_school_schedule_event_v1", "get_admin_school_schedule_edit_result_v1"].includes(rpc)) {
      ...
      return { status: result.status, category: "school-schedule-edit", body: result.body };
    }
