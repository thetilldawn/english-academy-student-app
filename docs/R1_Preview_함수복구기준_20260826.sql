-- Preview R1 함수 복구 기준 스냅샷
-- project: wojxpruvbjzbhrpmsbuy (staging/Preview only)
-- captured_at: 2026-08-26 02:45:56.201576+00
-- 비밀값과 학생 데이터는 포함하지 않는다.

-- signature: private.materialize_ready_vocab_assignment_queue_v1(p_student_id uuid, p_limit integer)
-- owner: postgres
-- acl: postgres=X/postgres
CREATE OR REPLACE FUNCTION private.materialize_ready_vocab_assignment_queue_v1(p_student_id uuid, p_limit integer DEFAULT 10)
 RETURNS jsonb
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO ''
...
-- signature: public.answer_quiz_question_v2(p_student_id uuid, p_attempt_id uuid, p_question_id uuid, p_phase text, p_choice_index smallint, p_force_timeout boolean)
-- owner: postgres
-- acl: postgres=X/postgres
-- acl: service_role=X/postgres
CREATE OR REPLACE FUNCTION public.answer_quiz_question_v2(p_student_id uuid, p_attempt_id uuid, p_question_id uuid, p_phase text, p_choice_index smallint, p_force_timeout boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
-- End of Preview recovery snapshot.
-- signature: public.finalize_expired_review_assignment_drafts(p_student_id uuid, p_limit integer)
-- owner: postgres
-- acl: postgres=X/postgres
-- acl: service_role=X/postgres
CREATE OR REPLACE FUNCTION public.finalize_expired_review_assignment_drafts(p_student_id uuid, p_limit integer DEFAULT 400)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
-- signature: public.finalize_quiz_attempt_if_stale(p_attempt_id uuid)
-- owner: postgres
-- acl: postgres=X/postgres
-- acl: service_role=X/postgres
CREATE OR REPLACE FUNCTION public.finalize_quiz_attempt_if_stale(p_attempt_id uuid)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
-- signature: public.finalize_stale_quiz_attempts(p_limit integer)
-- owner: postgres
-- acl: postgres=X/postgres
-- acl: service_role=X/postgres
CREATE OR REPLACE FUNCTION public.finalize_stale_quiz_attempts(p_limit integer DEFAULT 100)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
-- signature: public.materialize_ready_vocab_assignment_queue_v1(p_student_id uuid, p_limit integer)
-- owner: postgres
-- acl: postgres=X/postgres
-- acl: service_role=X/postgres
CREATE OR REPLACE FUNCTION public.materialize_ready_vocab_assignment_queue_v1(p_student_id uuid, p_limit integer DEFAULT 10)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
-- signature: public.resume_quiz_after_feedback_v2(p_student_id uuid, p_attempt_id uuid, p_next_question_id uuid, p_next_phase text, p_transition_remaining_milliseconds integer)
-- owner: postgres
-- acl: postgres=X/postgres
-- acl: service_role=X/postgres
CREATE OR REPLACE FUNCTION public.resume_quiz_after_feedback_v2(p_student_id uuid, p_attempt_id uuid, p_next_question_id uuid, p_next_phase text, p_transition_remaining_milliseconds integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
