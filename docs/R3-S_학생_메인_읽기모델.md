...
## Preview 완료 기록 — 2026-08-29

- 구현 커밋은 `8203479`다. 학생 페이지는 인증 뒤 `Suspense` 아래 Server Component에서 초기
  읽기와 포인트 읽기를 두 개만 병렬 시작하고, 완료 더보기만 작은 Client 영역과 POST Route
  Handler를 사용한다. 이전의 전체 배정 ID 수집과 여러 `.in(...)` 조회 파일은 제거했다.
- 로컬에서는 집중 15파일·67테스트, SQL 2파일·8테스트, 전체 324파일·1,743테스트, lint,
  typecheck, 기능 지도와 구조 검사가 통과했다. 21개 완료 고정 자료는 `10 → 10 → 1`로 연결되고
  같은 완료 시각·조회 중 새 완료에서도 누락·중복이 0건이다. 초기 Client 전달값은 현재 12건과
  완료 10건만 포함해 전환 전 전체 33건 직렬화보다 작다는 회귀검사를 추가했다.
- Preview `wojxpruvbjzbhrpmsbuy`에는 SHA-256
  `8CEE56FD459D41F1A798549216CB5D2ECD1CE8D91CDDC49A003908B4AB47D76E`인 migration을
  `20260829002600 add_student_dashboard_read_model`로 한 건만 적용했다. 내부 투영 1개와 공개 RPC
  2개는 모두 `SECURITY INVOKER + STABLE + 빈 search_path`이며 `public`, `anon`,
  `authenticated` 실행 권한은 없고 `service_role`만 실행 가능하다. 새 보안·성능 권고는 0건이다.
- 가짜 `프리뷰 검토 학생`의 실제 초기 응답은 현재 11건·완료 7건·29,869바이트였다. 실행 계획은
  16.114ms, 공유 버퍼 hit 2,512·read 0이었다. 추측 인덱스는 추가하지 않았다. R0의 학생 메인
  기능 자료 조회 11~13회는 초기 읽기 모델과 포인트 읽기 2회로 줄었다.
- Git 연동 Preview의 학생 화면에서 배포 스크립트 13개가 로컬 빌드 13개와 모두 일치했다. 로그인된
  가짜 학생의 `공개 예정 1 / 재시험·미통과 1 / 종료된 시험 1 / 현재 포인트 0`은 같은 학생의
  DB 결과와 일치했고, 구역 열기와 시험 카드도 정상이다. 브라우저 warning/error는 0건이며 검증 뒤
  인앱 브라우저 탭을 모두 닫았다.
- 로컬 `next build`는 컴파일과 TypeScript까지 통과했지만 공개 Preview 환경변수가 로컬 파일에 없어
  정적 생성 단계에서 중단됐다. 동일 산출물의 Vercel 환경 빌드는 위 13개 자산 일치와 실제 화면
  응답으로 확인했다. Production과 실제 학생은 변경하지 않았다.
