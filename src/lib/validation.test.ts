...
describe("일괄 단어 시험 입력 계약", () => {
  ...
  it.each(["canonical_example_to_headword"])(
    "%s 일정 제한 오류는 배포 환경이 아닌 실제 지원 범위를 안내한다", (questionMode) => {
      ...
      )?.message).toBe("예문 시험은 현재 시험일 없이 1회만 바로 배정할 수 있습니다.");
    },
  );

  it.each([["canonical_definition_to_headword",0],["canonical_headword_to_definition",100]])("%s는 공통 회차·날짜 계획을 지원한다",(questionMode,englishToKoreanRatio)=>{
    expect(bulkAssignmentPreviewSchema.safeParse({...preview,questionMode,englishToKoreanRatio}).success).toBe(true);
  });
