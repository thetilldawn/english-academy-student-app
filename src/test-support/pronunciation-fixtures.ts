...
export function nucleusFixture() {
...
export function schoolPronunciationFixture(index = 0) {
  const scope = SCHOOL_PRONUNCIATION_SCOPES[index];
  const release = nucleusFixture();
  release.dataset_key = scope.datasetKey;
  release.dataset_source_sha256 = scope.sourceSha256;
  release.bindings = release.bindings.slice(0, scope.entryCount);
  release.summary.expected_entry_count = scope.entryCount;
  release.summary.binding_count = scope.entryCount;
  release.summary.webster_binding_count = scope.entryCount;
