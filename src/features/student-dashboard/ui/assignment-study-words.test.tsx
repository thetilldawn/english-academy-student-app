...
const study: AssignmentStudy = { assignmentId: "test", title: "학습장", mode: "canonical_example_to_headword", words: [{ key: "word", headword: "collect", meaning: "모으다", definition: null, example: "She collected the letters.", exampleRanges: [{ start: 4, end: 13 }], pronunciation: { displayKo: "컬렉트", audioUrl: url, available: true, variantId: null } }] };
...
describe("단어장 음성·학습 표시", () => {
  ...
  it("음성 차단·실패를 표시하고 다시 누르면 오류를 해제한다", async () => {
    ...
    await act(async () => fireEvent.click(screen.getByRole("button", { name: /단어 발음 듣기/u })));
    expect(screen.getByRole("alert")).toHaveTextContent("음성을 재생하지 못했습니다");
    await act(async () => fireEvent.click(screen.getByRole("button", { name: /단어 발음 듣기/u })));
    expect(screen.queryByRole("alert")).toBeNull();
  });
  it("없는 음성을 임의 생성하지 않고 비활성 스피커와 안내를 표시한다", () => {
    render(<AssignmentStudyReader presentation="page" study={{ ...study, words: [{ ...study.words[0]!, pronunciation: { available: false, audioUrl: null, displayKo: null, variantId: null } }] }} />);
    expect(screen.getByRole("button", { name: /단어 발음 듣기/u })).toBeDisabled();
    expect(screen.getByText(/발음 자료는 아직 준비되지/u)).toBeVisible();
  });
  it("영영풀이형에서는 풀이만 추가하고 예문을 섞지 않는다", () => {
    render(<AssignmentStudyReader presentation="page" study={{ ...study, mode: "canonical_definition_to_headword", words: [{ ...study.words[0]!, definition: "to gather things" }] }} />);
