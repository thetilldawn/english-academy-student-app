...
- 공개 판정은 DB 공용 함수 → `lib/assignment/assignment-release.ts` 계약을 사용한다.
  lifecycle/구역/카드/단어장에 공개 규칙을 복사하지 않는다. APP-20260907-04부터 앞 첫시험 완료와
  다음 자체 공개/마감만 유지하며 앞 마감+12시간 조건은 폐기했다. 재시험 완료는 기다리지 않는다.
...
- 구성 단어장의 학습 화면은 배정 때 저장된 뜻·영영풀이·발음을 사용한다. 고정값이 없는 발음은 빈 값으로 유지하며 현재 사전 조회는 기존 자료 행에만 적용한다. 문항 보기와 정답 위치는 학습 응답에 넣지 않는다.
## 학교 일정 연결

- 학교 일정과 이름 옆 시험 표시는 school-schedules의 공개 서버/UI를 사용한다. 학생 ID 입력 없이 현재 학생 세션으로 조회하며 같은 서버 요청에서만 중복 제거한다. 실제 시험 내역과 별도 구역에서 실패를 복구하고, 한국 자정에는 날짜 표시만 갱신한다.
