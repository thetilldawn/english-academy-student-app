...
import { Field, FieldError, FieldLabel, FieldLabelRow, FieldRequirement, Input } from "@/design-system/primitives/form/field";
import type { SchoolSearchController } from "../controller/use-school-search";
import styles from "./school-search-field.module.css";

export function SchoolSearchField({ controller, required = false, error }: { controller: SchoolSearchController; required?: boolean; error?: string }) {
  const id = useId();
  return <Field>
    <FieldLabelRow><FieldLabel htmlFor={id}>학교</FieldLabel>
      {required ? <FieldRequirement data-kind="required">필수</FieldRequirement> : null}</FieldLabelRow>
    <Input id={id} name="schoolName" autoComplete="off" maxLength={120} value={controller.value}
      required={required} aria-invalid={!!error} aria-describedby={`${id}-status${error ? ` ${id}-error` : ""}`} disabled={controller.locked}
