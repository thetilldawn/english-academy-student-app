...
- 학교 검색은 `use-school-search → transport/school-search → /api/admin/schools/search → school-search-query` 순서다. 등록/정보 수정은 같은 `SchoolSearchField`를 사용한다. 학교명과 공식 학교 선택키를 함께 저장하고 직접 이름을 바꾸면 선택키를 해제한다. 학생 추가는 공식 학교급 확인 후 1~3학년을 고르며 저장값은 중/고 접두를 유지한다. API 인증키는 서버 전용이고 외부 전달은 검색어만,350ms 입력 합치기와 전체 남은7초/본문 취소를 적용한다. 현재 요청 인증 거절만 개인정보를 숨긴다. 없음/장애는 구분하며 기존 값 자동 검색/저장/학년 추정은 하지 않는다.
- 학교 일정은 `school-schedules/public-server`로 목록 한 페이지를 묶어 조회한다. 상세·정상 저장·409·결과 확인은 현재 프로필과 일정이 같은 학교/학년인지 비교한다. 일정 실패는 목록/내역을 숨기지 않으며 상세의 결과 확인으로 초안을 보존한 채 복구한다.
...
- APP-20260908-02는 APP-20260907-01의 배정 전용 표시 유지를 학생 관리에도 확대한다. 현재 인증 ticket의 마지막 성공 목록은 60초가 지났다는 이유만으로 지우지 않는다. 목록 신선도/수동 갱신과 상세창·검색 입력 수명을 분리한다. 가로채기 상세 왕복은 배경 본문 이동이 아니지만 실제 본문 이동·복귀·계정·401/403 보호는 유지한다. 내역 목록의 기본 표시 상한은 이 변경 대상이 아니다.
...
- APP-20260908-02: 프로필 저장 결과는 모달 안에 남긴다. 미인증과 인증 조회 장애, 충돌, 저장 결과 불명을 구분한다. 결과 불명에는 읽기 전용 결과 확인을 먼저 제공하며 자동 재저장을 금지한다. 마이크로초 버전 문자열을 변환하지 않는다. 저장401은 이름 헤더까지 숨긴다. 배정 학생 이름은 공용 상세의 `tab=history` 진입이고 직접 학생 목록의 기본 정보 탭은 유지한다.
