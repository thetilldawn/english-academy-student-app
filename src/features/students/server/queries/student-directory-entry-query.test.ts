...
import { getStudentDirectoryCacheSeed } from "./student-directory-entry-query";
...
describe("최초 문서 인계", () => {
  it.each([["sec-fetch-dest", "document"], ["accept", "text/html"]])("현재 서버 인증/목록을 한 번만 읽는다: %s=%s", async (key, value) => {
    ...
    expect(await getStudentDirectoryCacheSeed(filters)).toMatchObject({ kind: "snapshot", userId: "fake-admin" });
  ...
  it("Client 탐색에는 예전 목록이나 인증 결과를 새 인계로 만들지 않는다", async () => {
    ...
    expect(await getStudentDirectoryCacheSeed(filters)).toBeUndefined(); expect(mocks.admin).not.toHaveBeenCalled(); expect(mocks.initial).not.toHaveBeenCalled();
  });
  it("문서 헤더도 권한을 대신하지 않는다", async () => {
    ...
    await expect(getStudentDirectoryCacheSeed(filters)).rejects.toThrow("not authorized"); expect(mocks.initial).not.toHaveBeenCalled();
  });
});
