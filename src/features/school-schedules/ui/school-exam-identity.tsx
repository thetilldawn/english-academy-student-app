...
import { nearestSchoolExam, isUpcomingSchoolEvent } from "../domain/school-schedule";
...
export function SchoolExamIdentity({ children, summary, refreshNeeded = false, as: Tag = "div" }: {
  ...
  const unknownExam = summary?.events.some(event => event.kind === "written" && !event.subjectDate && isUpcomingSchoolEvent(event, summary.today));
  ...
  return <Tag className={styles.identityContainer}><Tag className={styles.identity}>
    {summary ? <span className={styles.countdown} data-urgent={next !== null && next.days <= 14}
      title={next ? `${next.exam.title} / ${next.dates.startDate} / ${next.exam.kind === "csat" ? "수능 시행일 기준" : "영어 시험일 기준"}` : label}>
      {label}
    </span> : null}
