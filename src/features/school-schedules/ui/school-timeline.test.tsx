...
it("학생 본인에게 편집 링크나 다른 학교 학생 수를 노출하지 않는다",()=>{
  ...
  expect(screen.getByText("날짜 확인 중")).toBeVisible();expect(screen.queryByLabelText("평가 유형별 학생 수")).toBeNull();
});
it("겹친 일정은 한 요소에 나열하고 다음 날짜는 펼침 목록에 둔다",()=>{
  ...
  render(<SchoolTimeline overview={{status:"ready",today:summary.today,groups:[{studentCount:2,summary:{...summary,events:[
    event,{...event,id:"p",kind:"performance",title:"가상 수행",startDate:"2026-10-14",endDate:"2026-10-14"},
    {...event,id:"later",title:"이후 지필",startDate:"2026-12-01",endDate:"2026-12-01"}]}}]}} />);
  const first=screen.getByRole("article",{name:"10/12 일정"});expect(within(first).getByText("가상 수행")).toBeVisible();
  expect(within(first).getByText("2학기 1차 시험")).toBeVisible();expect(screen.getByText("이후 지필")).not.toBeVisible();
});
...
it("주차는 날짜를 만들지 않고 예정 태그와 편집 링크를 표시한다",()=>{
  ...
  expect(screen.getByRole("link",{name:"10월 4주 / 날짜 입력"})).toBeVisible();expect(screen.getByText("예정")).toBeVisible();
