...
- APP-20260919-01: 이미 인증된 같은 화면의 내역은 `retainActiveSnapshot`으로 시간 경과 뒤에도 유지한다. 60초·120초 경과나 @detail 닫기로 정상 목록을 오류로 바꾸거나 새로 조회하지 않는다. 새로고침 버튼은 추가하지 않는다. 생성·수정·취소·숨김 성공의 기존 변경 알림, 실제 본문/탭 복귀 재인증, 401/403 차단, 실제 실패의 재시도와 이전 커서 폐기는 유지한다.
