...
import type { BulkAssignmentInput } from "@/features/assignments/contracts/bulk-assignment-request";
...
export function buildVocabAssignmentQueueSeriesPayload(input: {
  ...
  if (
    ...
    input.commonPlan.recurrenceSessions.some(
      (slot) => !slot.availableFrom || !slot.availableUntil,
    )
  ) {
    throw new VocabAssignmentQueuePlanError(
      "완료 뒤 다음 시험 생성에는 공개·마감 일정이 필요합니다.",
    );
  }
  ...
  const recurrenceSlots = input.commonPlan.recurrenceSessions.map((slot) =>
