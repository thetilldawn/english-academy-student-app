...
describe("영구 시간 템플릿", () => {
  it("저장한 템플릿이 없으면 기본 버튼을 미리 만들지 않는다", () => {
    ...
    expect(result.current.timeTemplates).toEqual([]);
  });
  ...
  it("서버 저장에 성공한 템플릿만 현재 버튼 목록에 추가한다", async () => {
    ...
    await act(async () => {
      expect(await result.current.saveCurrentTemplate("저녁"))
    ...
    expect(result.current.customTemplates[0]?.label).toBe("저녁");
  });
  ...
  it("저장된 버튼과 같은 이름은 서버에 보내지 않는다", async () => {
    ...
    await act(async () => {
      expect(await result.current.saveCurrentTemplate("저녁 수업")).toEqual({
        ok: false,
