...
- 단일·일괄 범위 배정은 같은 `BulkAssignmentCommonPlan` 계약을 쓰되 `audienceMode`로 구분한다.
  단일의 학년 차이는 의도된 배정으로 허용하며 일괄은 1명이 남아도 포함/제외를 확인한다.
  `domain/assignment-grade-review.ts`가 명확한 학년만 비교하고 `server/planning/assignment-grade-review.ts`가
  저장 직전 현재 학년·학생·단어장을 재확인한다. 학생/단어장 변경은 확인을 지우며, 제외는 새 배정 대상만
  바꾸고 기존 시험·응시·점수는 수정하지 않는다. 포함한 예문과 영영풀이는 선택 단어장 기준을 유지한다.
