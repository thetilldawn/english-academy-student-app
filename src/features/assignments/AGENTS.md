...
- APP-20260921-03: 독립 오답 출제는 후보 전체·가능·불가 및 이유를 구분한다. 부분 출제는 현재 미리보기의 제외 확인을 요구하고 학생/조건/후보/문항판이 바뀌면 다시 확인한다. v3 저장은 전체 후보 일치와 출제/제외 분할·자료 확인값을 한 거래에서 검사하며 완료 요청 재반환과 구형 전체 출제의 엄격한 비교를 보존한다.

- APP-20260921-02: `assignment-student-context`에서 선택 학생의 공통 학교/학년과 현재 학기를 기본값으로 계산한다. 같은 대상에서 직접 바꾼 조건은 보존하고 새 구성 대상만 전달한다. 필수 정보 누락은 새 배정 전 화면/미리보기/최종 DB 거래에서 거절한다. 프로필 저장 알림은 `students/public-client`로 받아 열린 준비 자료의 학생 필드만 바꾸며 전체 준비 재조회로 범위를 잃지 않는다. 완료 요청 결과 확인은 새 필수 검사보다 먼저 유지한다.
...
- 구성 단어장의 후보는 고정 문항 ID·확인값으로 배정한다. 회차별 범위는 각 회차의 실제 단원 집합으로 모호성을 검사하고, 아직 배정하지 않은 범위도 같은 단위 수 규칙으로 전체·남은 수에 포함한다. 단어 수별/전체 범위 방식은 선택 전체를 검사한다. 여러 회차의 한쪽 방향씩을 합쳐 양방향 출제 가능으로 승격하지 않는다.
- 고정 구성의 원범위 선택은 겹치는 부분 범위의 모든 단원 소속을 유지한다. 같은 단어장의 일반 배정·오답·설정 수정은 고정 문항과 발음을 사용하고 범위 밖 보기는 대상 단원 수에 포함하지 않는다.

- APP-20260921-04: 원자료 기준 문항 순서는 ascending/descending/random 세 값을 공통 입력·일반·일괄·오답·편집·이전 조건 복사에서 보존한다. 기존 fixed만 ascending으로 호환하며 알파벳 정렬로 바꾸지 않는다.
