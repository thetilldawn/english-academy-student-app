...
begin
  ...
  then
    raise exception 'question_bank_v2_provenance_rollback_shape_changed';
  end if;

  function_definition := regexp_replace(
    function_definition,
    runtime_pattern,
    E'and eligibility.status = ''eligible''\n    and eligibility.rule_version = selected_rule_version;',
    'i'
  );
  ...
  then
    raise exception 'question_bank_v2_provenance_rollback_rewrite_failed';
  end if;
  ...
  if not exists (
    ...
    raise exception 'question_bank_v2_provenance_rollback_metadata_changed';
  end if;
end;
$rollback$;
