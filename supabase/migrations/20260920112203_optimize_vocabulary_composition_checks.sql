-- Lean question-generation input keeps full frozen resources inside the database.
...
begin
  ...
  if found then return private.vocabulary_composition_response_v1(p_version_id,true); end if;
...
$$;
create function private.vocabulary_composition_question_input_v1(p_version_id uuid)
returns jsonb language sql stable security definer set search_path='' as $$
  select jsonb_build_object('versionId',c.version_id,'datasetId',c.dataset_id,'contentHash',c.content_sha256,'state',c.state,
    'entries',coalesce((select jsonb_agg(jsonb_build_object('id',e.id,'unitId',e.unit_id,'sourceRow',e.source_row,
      ...
      'eligibleDirections',l.eligible_directions,'compositionTargetKey',l.identity_key) order by e.source_row)
...
begin
  ...
  perform private.prepare_vocabulary_composition_data_v1(version_value,p_request->>'contentHash',auth.uid(),null);
  result:=private.vocabulary_composition_question_input_v1(version_value);
...
begin
  result:=private.prepare_vocabulary_composition_data_v1(p_version_id,p_content_sha256,p_actor_id,p_management_request_id);
  if p_management_request_id is not null then return result; end if;
  return private.vocabulary_composition_preparation_v1(p_version_id);
end;
$$;
create function public.finalize_vocabulary_composition_summary_v1(p_version_id uuid,p_content_sha256 text,p_questions jsonb)
...
begin
  ...
  return private.finalize_vocabulary_composition_core_v1(p_version_id,p_content_sha256,p_questions,true);
end;
$$;
alter function public.prepare_vocabulary_template_question_input_v1(jsonb) set statement_timeout='55s';
revoke all on function private.prepare_vocabulary_composition_data_v1(uuid,text,uuid,uuid),private.vocabulary_composition_question_input_v1(uuid),
  public.prepare_vocabulary_template_question_input_v1(jsonb),public.finalize_vocabulary_composition_summary_v1(uuid,text,jsonb) from public,anon,authenticated,service_role;
grant execute on function public.prepare_vocabulary_template_question_input_v1(jsonb) to authenticated;
grant execute on function public.finalize_vocabulary_composition_summary_v1(uuid,text,jsonb) to service_role;
