-- Read each large immutable JSON value once before joining small row metadata.
create or replace function private.vocabulary_composition_positions_v1(p_fixed jsonb)
...
language sql stable security definer set search_path='' as $$
  with memberships as materialized (
    select key,array(select value::uuid from jsonb_array_elements_text(m.value)) scope_ids
    from jsonb_each(p_fixed->'rowScopes') m
  ), picked as materialized (
    select x.ordinality::integer row_no,x.value occurrence_key,m.scope_ids
    from jsonb_array_elements_text(p_fixed->'includedKeys') with ordinality x join memberships m on m.key=x.value
  ), headers as materialized (
    select s.id,s.dataset_id,s.source_kind,s.source_release_id,s.source_version,s.source_file_sha256,s.unit_id,
      (s.payload->'classification')-array['school','targetGrade','schoolYear','semester','assessment','purpose'] class
    from private.vocabulary_library_scopes s where s.id in(select scope_ids[1] from picked)
    ...
    select p.*,s.id scope_id,jsonb_build_array(s.dataset_id,s.source_kind,s.source_release_id,s.source_version,s.source_file_sha256,s.unit_id,
      s.class,to_jsonb(array(select x from unnest(p.scope_ids) x order by x))) block_key
    from picked p join headers s on s.id=p.scope_ids[1]
  ), changed as (
