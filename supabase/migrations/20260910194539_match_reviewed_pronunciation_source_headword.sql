begin;
-- DEPLOY-20260911-01: explicit, evidence-bound source label for reviewed alternatives.
alter table private.entry_source_pronunciations_v1
  add column identity_headword text,
  add constraint entry_source_identity_headword_proof_check check (
    identity_headword is null or (source_kind='identity' and identity_headword<>headword
      and identity_headword=btrim(identity_headword) and length(identity_headword) between 1 and 200)
  );
 ...
 with requested as materialized (
   ...
   where s.source_kind='identity' and exists (
     ...
     where a.vocab_entry_id=s.vocab_entry_id and i.identity_id=s.identity_id
       and i.headword=coalesce(s.identity_headword,s.headword) and i.lexical_pos=coalesce(s.identity_lexical_pos,s.lexical_pos)
       ...
       and (
         (s.identity_lexical_pos is null and s.identity_headword is null and exists (select 1 from public.vocab_entry_pronunciation_bindings_v2 b
