drop function if exists public.preflight_vocab_link_lexeme_batch(
  uuid,
  jsonb
);
drop function if exists private.preflight_vocab_link_lexeme_batch(
  uuid,
  jsonb
);
...
begin
  ...
  if not exists (
    ...
    from word_index.index_build as build
    where build.build_id = p_build_id
      and upper(build.input_snapshot_sha256) = p_input_snapshot_sha256
  ) then
    raise exception 'vocab_link_lexeme_preflight_snapshot_mismatch'
      using errcode = '21000';
  end if;
